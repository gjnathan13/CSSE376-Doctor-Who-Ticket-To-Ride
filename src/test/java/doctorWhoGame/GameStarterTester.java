...
public class GameStarterTester {
	...
	public void testStartButtonCreation() throws Exception {
		Method startButtonMaker = testStarter.getClass().getDeclaredMethod("createStartButton", BufferedImage.class,
				Image.class);
		startButtonMaker.setAccessible(true);

		BufferedImage startButtonImage;
		if (GraphicsEnvironment.isHeadless()) {
			startButtonImage = new BufferedImage(100, 100, BufferedImage.TYPE_INT_RGB);
		} else {
			startButtonImage = ImageIO.read(new File("GameImages\\StartButtonImage.png"));
		}
	...
	public void testTitleLabelCreation() throws Exception {
		Method labelMaker = testStarter.getClass().getDeclaredMethod("createStartLabel", int.class, int.class,
				Image.class);
		labelMaker.setAccessible(true);

		BufferedImage startBack;
		if (GraphicsEnvironment.isHeadless()) {
			startBack = new BufferedImage(100, 100, BufferedImage.TYPE_INT_RGB);
		} else {
			startBack = ImageIO.read(new File("GameImages\\TitleImage.png"));
		}
